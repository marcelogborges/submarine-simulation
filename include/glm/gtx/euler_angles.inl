...
namespace glm
{
	...
	{
		T tmp_ch = glm::cos(rotate_z);
		T tmp_sh = glm::sin(rotate_z);
		T tmp_cp = glm::cos(rotate_y);
		T tmp_sp = glm::sin(rotate_y);
		T tmp_cb = glm::cos(rotate_x);
		T tmp_sb = glm::sin(rotate_x);
	...
	template<typename T>
	GLM_FUNC_QUALIFIER mat<4, 4, T, defaultp> rotate
	(
		T const& rotate_z,
		T const& rotate_y,
		T const& rotate_x
	...
	{
		return mat<3, 3, T, Q>(rotate(angles.z, angles.x, angles.y));
	}
	...
	{
		return rotate(angles.z, angles.x, angles.y);
	}
